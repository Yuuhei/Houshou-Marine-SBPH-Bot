...
class Qdl(commands.Cog, name="qdl"):
    ...
    # -----------------------------------------------------------------------------------------------------------------
    @commands.has_any_role("Horni Supporter")
    async def dl(self, ctx, link, quality):
        """
        Downloads music from Qobuz. Check #how-to-use-bots for instructions how to use this command.
        """
        ...
        if ctx.channel.id == request_channel:
            if link.find("artist") != -1:
                ...
                await ctx.reply(f"{ctx.author.mention}, downloading **Artist Profile** and **Playlists** not allowed for now.")
            # ------------ EDIT THESE LINES IF QOBUZ EXPIRES ---------------------
            # --------------------------------------------------------------------
            # elif link.find("qobuz") != -1:
                # await ctx.reply(f"{ctx.author.mention}, **Qobuz** downloads are disabled for now, as our subscription on this service is currently expired. Sorry for inconvenience, we'll be right back soon!")
            # --------------------------------------------------------------------
            # --------------------------------------------------------------------
            elif link.find("music.apple") != -1:
                await ctx.reply(f"{ctx.author.mention}, please use `h!dl` command for non-Qobuz links.")
            ...
            # elif link.find("soundcloud") != -1:
                ...
                await ctx.reply(f"{ctx.author.mention}, use `h!spotdl <spotify link here>` Spotify links.")
                ...
                try:
                    ...
                    with open('rip_log.txt', 'wb') as f:
                        await ctx.message.add_reaction('📥')
                        process = subprocess.Popen(["rip", 'url', f'{link}', "--max-quality", f'{quality}', '-i'], stdout=subprocess.PIPE)
