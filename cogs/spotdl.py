...
class SpotDL(commands.Cog, name="spotdl"):
    ...
    async def dl(self, ctx, link):
        """
        Downloads music from Spotify. Check #how-to-use-bots for instructions how to use this command.
        """
        ...
        if ctx.channel.id == request_channel:
            if link.find("artist") != -1:
                await ctx.reply(f"{ctx.author.mention}, downloading **Spotify Artist Profile and Playlists** not allowed for now.")
            ...
            # elif link.find("spotify") != -1:
                # await ctx.send(f"{ctx.author.mention}, **Spotify** downloads is disabled for now due to a major bug that causes the bot to crash when no matching track and metadata is found.\nThis is caused by the downloader used by the bot SpotDL, there's nothing i can do about it and we'll wait for their bugfix.")
                ...
                try:
                    ...
                    with open('spotdl_log.txt', 'wb') as f:
                        await ctx.message.add_reaction('📥')
                        process = subprocess.Popen(["spotdl", "download", f'{link}', "--output", f"{download_folder}download/Temp/", "--preload", "--sponsor-block", "--print-errors", "--format", "m4a"], stdout=subprocess.PIPE)
                        for line in iter(process.stdout.readline, b''):
                            sys.stdout.buffer.write(line)
                            f.write(line)

                    # Deletes spotdl cache for correct filename
                    # subprocess.run(["rm", f"{download_folder}download/Temp/.spotdl-cache"])
                    ...
                    # Uncomment to attach QR Code
                    # -----------------------------------------------------------------------------------------------
                    # Attach QR Code for easier downloads
                    ...
                    all_done.add_field(name="Download Link (Plain Text for easier copying below this embed)", value=gdrive_b64, inline=False)
